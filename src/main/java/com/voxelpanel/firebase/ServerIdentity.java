...
public class ServerIdentity {
    ...
    public ServerIdentity(VoxelPanel plugin) {
        this.plugin = plugin;
    ...
    private void load() {
        // Prefer values provided by the panel via config.yml. New configs use
        // web.token / firebase.server-id; legacy configs used firebase.auth-token.
        String cfgServerId = plugin.getConfig().getString("firebase.server-id", "");
        String cfgToken = plugin.getConfig().getString("web.token",
                plugin.getConfig().getString("firebase.auth-token", ""));
    ...
    /**
     * Validates the panel connection settings and logs any problems.
     * Never throws — returns false so the caller can stand down gracefully.
     */
    public boolean validateConfig() {
        boolean ok = true;
        // New key is web.url; fall back to legacy firebase.database-url.
        String dbUrl = plugin.getConfig().getString("web.url",
                plugin.getConfig().getString("firebase.database-url", ""));
        if (dbUrl == null || dbUrl.isBlank() || dbUrl.contains("YOUR-PROJECT")) {
            plugin.getLogger().warning("[VoxelPanel] config.yml: web.url is missing or a placeholder.");
            ok = false;
        }
        if (serverId == null || serverId.isBlank()) {
            plugin.getLogger().warning("[VoxelPanel] config.yml: firebase.server-id is empty.");
            ok = false;
        }
        int cfgVersion = plugin.getConfig().getInt("firebase.config-version", 0);
        if (cfgVersion < 1) {
            plugin.getLogger().warning("[VoxelPanel] config.yml: firebase.config-version looks outdated. Re-copy the config from the panel.");
        }
        if (!hasDashboardToken()) {
            plugin.getLogger().info("[VoxelPanel] No web.token set — running in legacy pairing mode (enter the pairing code in the panel).");
        }
        return ok;
    }
    ...
    public void printBanner() {
        ...
        plugin.getLogger().info("  VoxelPanel Dashboard — Server Connection");
