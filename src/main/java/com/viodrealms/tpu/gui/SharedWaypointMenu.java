...
public class SharedWaypointMenu {
    ...
    public static void open(Player player, ViodRealmsTPU plugin, int page) {
        GUIUtils.cancelAnimation(player.getUniqueId());
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            List<Waypoint> waypoints = plugin.getWaypointRepository().getSharedWaypointsForPlayer(player.getUniqueId());
            plugin.getServer().getScheduler().runTask(plugin, () -> render(player, waypoints, plugin, page));
        });
    }

    private static void render(Player player, List<Waypoint> waypoints, ViodRealmsTPU plugin, int page) {
        Inventory inventory = Bukkit.createInventory(null, 54, ChatColor.DARK_GRAY + "Shared Waypoints");
        ...
        if (currentPage > 0) {
            inventory.setItem(48, GUIUtils.button(Material.ARROW, "&fPrevious Page",
                    List.of("&7Page " + currentPage + " / " + totalPages), "shared_page:" + (currentPage - 1)));
        }
        ...
        if (currentPage < totalPages - 1) {
            inventory.setItem(50, GUIUtils.button(Material.ARROW, "&fNext Page",
                    List.of("&7Page " + (currentPage + 2) + " / " + totalPages), "shared_page:" + (currentPage + 1)));
        }
