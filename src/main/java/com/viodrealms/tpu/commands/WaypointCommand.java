...
public final class WaypointCommand implements CommandExecutor {
    ...
    public WaypointCommand(ViodRealmsTPU plugin, WaypointManager waypoints, MessageManager messages, SoundManager sounds) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                ...
                Bukkit.getScheduler().runTask(plugin, () -> {
                    plugin.getShareRequestManager().createRequest(target.getUniqueId(),
                            new com.viodrealms.tpu.managers.ShareRequestManager.ShareRequest(wpId, player.getUniqueId(), player.getName(), wpName));
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("export")) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                ...
                int count = com.viodrealms.tpu.utils.ImportExportUtil.exportWaypoints(plugin, player.getUniqueId(), fileName, wps);
                Bukkit.getScheduler().runTask(plugin, () -> {
                    if (count >= 0) {
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("import")) {
            String fileName = args[1].replaceAll("[^A-Za-z0-9_-]", "_");
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                var imported = com.viodrealms.tpu.utils.ImportExportUtil.importWaypoints(plugin, player.getUniqueId(), player.getName(), fileName);
                if (imported == null) {
                ...
                int count = 0;
                for (var wp : imported) {
                    if (existing + count >= maxWaypoints && !player.hasPermission("viodrealms.tpu.bypass.limit")) {
                        break;
                    }
